...
//			Console.WriteLine ("returning(" + lzwCodeSize + ") " + val);    
	...
	int loc = ((y - top) * w) + (x - left); 
int cr = (int)localColorTable[(int)output [loc]][0]; 
int cg = (int)localColorTable[(int)output [loc]][1]; 
int cb = (int)localColorTable[(int)output [loc]][2]; 
int cidx = (cr + cg + cb) * 12 / 256; 
  
if(cidx >= 0 && cidx <= 3) 
{ 
int loc2 = (y * (width + 1)) + x; 
if(loc2 < frame.Length) 
{ 
if(cidx == 0) 
    			frame[loc2] = '\uE00F';  
else if (cidx == 1) 
